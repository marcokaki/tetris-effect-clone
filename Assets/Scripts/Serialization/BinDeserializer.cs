...
public ref struct BinDeserializer
{
	public BinDeserializer(System.ReadOnlySpan<byte> buf) {
		_buf = buf;
		_index = 0;
	}
	...
	public void io_object(ref object obj) {
		...
		if (type.IsArray) {
			...
			for (int i = 0; i < len; i++)
			{
				var elemValue = _createInstance(elemType);
		...
		if (dict != null) {
			...
			for (int i = 0; i < len; i++)
			{
				...
				var value = _createInstance(valueType);
				io_object(ref key);
				io_object(ref value);
				dict.Add(key, value);
			}
		...
		foreach (var f in fields) {
			var fieldValue = _createInstance(f.FieldType);
	...
	public void io_fixed<T>(ref T value) {
		var type = typeof(T);
		object obj = value;
		io_object_fixed(ref obj);
		value = (T)obj;
    }

	public void io_object_fixed(ref object obj)
    {
		var type = obj.GetType();
		if (type == typeof(byte  )) { byte   tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(ushort)) { ushort tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(uint  )) { uint   tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(ulong )) { ulong  tmp = 0; io_fixed(ref tmp); obj = tmp; return; }

		if (type == typeof(sbyte )) { sbyte  tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(short )) { short  tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(int   )) { int    tmp = 0; io_fixed(ref tmp); obj = tmp; return; }
		if (type == typeof(long  )) { long   tmp = 0; io_fixed(ref tmp); obj = tmp; return; }

		var fields = type.GetFields();
		foreach (var f in fields) {
			var fieldValue = _createInstance(f.FieldType);
			io_object_fixed(ref fieldValue);
			f.SetValue(obj, fieldValue);
		}
	}
	...
	object _createInstance(System.Type type)// <== added for nested array, string field etc.
    {
		if (type == typeof(string)) return string.Empty;

        if (type.IsArray) {
			var elemType = type.GetElementType();
			return System.Array.CreateInstance(elemType, 0);
        }

		return System.Activator.CreateInstance(type);
    }

	readonly System.ReadOnlySpan<byte> _buf;
	int _index;
}