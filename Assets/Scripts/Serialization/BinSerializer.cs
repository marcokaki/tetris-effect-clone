...
public ref struct BinSerializer
{
	...
	public void io_fixed(ref ushort value, int writingIndex) {
		_buf[writingIndex	 ] = ((byte) value);
		_buf[writingIndex + 1] = ((byte)(value >> 8));
	}
	...
	public void io_object_fixed(object obj)//for enum only
    {
		var type = obj.GetType();
		if (type == typeof(byte  )) { byte   tmp = (byte  )obj; io_fixed(ref tmp); return; }
		if (type == typeof(ushort)) { ushort tmp = (ushort)obj; io_fixed(ref tmp); return; }
		if (type == typeof(uint  )) { uint   tmp = (uint  )obj; io_fixed(ref tmp); return; }
		if (type == typeof(ulong )) { ulong  tmp = (ulong )obj; io_fixed(ref tmp); return; }

		if (type == typeof(sbyte )) { sbyte  tmp = (sbyte )obj; io_fixed(ref tmp); return; }
		if (type == typeof(short )) { short  tmp = (short )obj; io_fixed(ref tmp); return; }
		if (type == typeof(int   )) { int    tmp = (int   )obj; io_fixed(ref tmp); return; }
		if (type == typeof(long  )) { long   tmp = (long  )obj; io_fixed(ref tmp); return; }
